...
//! MLX TTS module - uses moshi_mlx Python package for Apple Silicon acceleration
...
impl Inner {
    ...
    fn start_model_loop(self, batch_size: usize) -> Result<()> {
        ...
        crate::utils::spawn_blocking("mlx_model_loop", move || {
            tracing::info!("starting-up the MLX model loop");
                    ...
                    c.par_iter_mut().enumerate().for_each(|(batch_idx, channel)| {
                        if let Some(c) = channel.as_mut() {
                            ...
                            if (mask & MASK_IS_EOS) > 0 {
                                c.on_end_of_word();
                                tracing::info!(?batch_idx, "mlx tts finished");
                                *channel = None;
                            }
                        }
                    });
...
impl M {
    pub fn new(config: crate::MlxConfig) -> Result<Self> {
        init()?;
        let text_tokenizer =
        ...
        let (script, script_name) = match &config.script {
            None => {
                let script_name = std::ffi::CString::new("tts_mlx.py")?;
                let script = std::ffi::CString::new(crate::TTS_MLX_PY)?;
                (script, script_name)
            }
            Some(script) => {
        ...
        let app = Python::with_gil(|py| -> Result<_> {
            ...
            // Add voice_folder and default_voice to py config
            py_config.set_item("voice_folder", &config.voice_folder)?;
            py_config.set_item("default_voice", &config.default_voice)?;
            // Add quantization if specified
            if let Some(quantize) = config.quantize {
                py_config.set_item("quantize", quantize)?;
            }
            ...
            let app =
                PyModule::from_code(py, script.as_c_str(), script_name.as_c_str(), c_str!("tts_mlx"))
                    .map_err(VerbosePyErr::from)?
                    .getattr("init")?
    ...
    pub async fn handle_query(
        ...
        tracing::info!(batch_idx, "mlx channel");
        ...
        crate::utils::spawn("mlx_send_loop", async move {
    ...
    pub async fn handle_socket(&self, socket: ws::WebSocket, query: Query) -> Result<()> {
        ...
        tracing::info!(?query, "mlx query");
        metrics::CONNECT.inc();

        let (mut sender, receiver) = socket.split();
        let (bidx, in_tx, mut out_rx) = match self.channels(query.format, query.voice.clone(), 299792458)? {
            Some(x) => x,
            None => {
        ...
        tracing::info!(?bidx, "mlx channel");
        let mut text_tokenizer = crate::tts_preprocess::Tokenizer::new(
            self.text_tokenizer.clone(),
            self.config().text_bos_token,
        );

        crate::utils::spawn("mlx_recv_loop", async move {
    ...
    pub fn config(&self) -> &crate::MlxConfig {
        &self.config
    }
